    ...
    for (const msg of inboxMessages as InboxMessage[]) {
      ...
      for (const rule of rules as OtpRule[]) {
        ...
        if (rule.auto_detect) {
          ...
          // Fall back to smart auto-detection when the custom pattern didn't
          // match, so the OTP is extracted from any content (e.g. a pattern
          // like "Your code is (\d+)" vs "Your OTP code is 252525").
          if (!extractedOtp) extractedOtp = autoDetectOtp(msg.content);
        }
