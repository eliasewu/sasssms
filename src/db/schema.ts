...
export const voiceOtpConfig = pgTable("voice_otp_config", { id: serial("id").primaryKey(), countryGroup: varchar("country_group", { length: 255 }).notNull(), prefixes: varchar("prefixes", { length: 500 }), primaryLanguage: varchar("primary_language", { length: 50 }).notNull(), secondaryLanguage: varchar("secondary_language", { length: 50 }), primaryAudioCount: integer("primary_audio_count").default(0), secondaryAudioCount: integer("secondary_audio_count").default(0), playCount: integer("play_count").default(3), retryCount: integer("retry_count").default(1), bilingual: boolean("bilingual").default(false), isActive: boolean("is_active").default(true).notNull(), createdAt: timestamp("created_at").defaultNow().notNull() });
...
// ── Global SMPP Username Index (public schema, O(1) cross-tenant uniqueness) ──
export const smppUsernames = pgTable("smpp_usernames", {
  id: serial("id").primaryKey(),
  smppUsername: varchar("smpp_username", { length: 100 }).notNull().unique(),
  tenantId: integer("tenant_id").notNull(),
  clientId: integer("client_id").notNull(),
