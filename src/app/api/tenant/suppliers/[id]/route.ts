...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await tenantQuery(
    ...
    `UPDATE suppliers SET 
      ...
      currency=$22, force_dlr=$23, charging_mode=$24, dlr_timeout=$25, is_active=$26, config=$27, updated_at=NOW()
    ...
    [
      ...
      body.currency ?? 'USD',
      body.forceDlr ?? false, body.chargingMode ?? 'on_submit', body.dlrTimeout ?? null, body.isActive ?? true, body.config ?? null, id
    ]
  );
