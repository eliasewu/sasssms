...
export async function POST(request: Request) {
  ...
  const { supplierId, countryCode, mcc, mnc, cost } = body;

  const result = await tenantQuery(
    tenant.schemaName,
    `INSERT INTO supplier_rates (supplier_id, country_code, mcc, mnc, cost) VALUES ($1,$2,$3,$4,$5) RETURNING *`,
    [supplierId, countryCode, mcc || null, mnc || null, cost]
  );
