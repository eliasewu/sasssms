...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await tenantQuery(
    tenant.schemaName,
    "DELETE FROM supplier_rates WHERE id = $1 RETURNING id",
    [id]
  );
  ...
  await auditLog("supplier_rates", parseInt(id), "DELETE", tenant.email, oldData as Record<string, unknown> || undefined, undefined, tenant.tenantId);

  return NextResponse.json({ success: true, message: "Supplier rate deleted" });
}
